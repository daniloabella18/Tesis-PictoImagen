        ...
        <div class="row">
          <div class="col-md-12">
            <div class="title-area wow fadeInUp">
              <h2 class="title">Reporte <span>#2</span></h2>
              <?php
                echo "<p>Cantidad de interacciones en cuadros realizadas por ".$_SESSION["usuario"]." en cada actividad.</p>";
              ?>
            </div>
          </div>
        </div>
	...
	<div id="wrap">
		<table id=customers cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered">
          ...
          <th>Interacciones ↑↓</th>
			...
			<tbody>
        <?php
          $q = "SELECT Tea_nom, Tac_nom, COUNT(His_Cua_Act_cod) AS Interacciones
                FROM historial, actividad, tipoactividad, tema
                WHERE His_Usu_id = '".$_SESSION["usuario"]."' AND His_Cua_Act_cod = Act_cod AND Act_Tac_cod = Tac_cod AND Act_Tea_cod = Tea_cod
                GROUP BY Act_cod;";
